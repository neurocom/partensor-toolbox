...
/********************************************************************/
/**
* @file      GtcStochastic.hpp
* @details
* Implements the Stochastic General Tensor Completion(gtc stochastic).
* Make use of @c spdlog library in order to write output in
* a log file in @c "../log". In case of using parallelism
* with mpi, then the functions from @c GtcStochasticMpi.hpp will be called.
********************************************************************/

#ifndef PARTENSOR_GTC_STOCHASTIC_HPP
#define PARTENSOR_GTC_STOCHASTIC_HPP
...
namespace partensor
{
  inline namespace v1
  {
    ...
    {
      /*
       * Includes the implementation of Stochastic General Tensor Completion. Based on the given
       * parameters one of the overloaded operators will be called.
       */
      template <std::size_t TnsSize_>
      struct GTC_STOCHASTIC_Base 
      {
      ...
      struct GTC_STOCHASTIC : public GTC_STOCHASTIC_Base<TnsSize_>
      {
        ...
        // Variables that will be used in gtc stochastic implementations. 
         ...
         *  
         * @param  st  [in] Struct where the returned values of @c GtcStochastic are stored.
         */
         ...
         * @param  st  [in,out] Struct where the returned values of @c GtcStochastic are stored.
                           ...
                           Status                 &st   )
        {
          ...
          switch ( st.options.constraints[idx] ) 
          {
            ...
            { 
              dynamic_blocksize::StochasticNesterovMNLS(mv.factors_T, mv.tnsDims, mv.tnsX[idx], mv.offsets[idx], 
                                                        mv.c_stochastic_perc, st.options.max_nesterov_iter, st.options.lambdas[idx], idx);
              break;
            }
                              ...
                              Status                 &status)
        {
          ...
          int epoch = int(mv.tnsX[0].nonZeros() / (mv.blocksize[0] * mv.tnsDims[0])) + 1;
          ...
          {
            status.ao_iter++;
            std::cout << "iter: " <<  status.ao_iter << " -- fvalue: " << status.f_value << " -- relative_costFunction: " << status.rel_costFunction << std::endl;
            ...
            if (status.ao_iter % epoch == 0)
            {
                ...
                cost_function(mv, status);
                status.rel_costFunction = status.f_value/status.frob_tns;
                // std::cout << epoch_counter << " - " << f_value << " - " << f_value / frob_tns << " - " << frob_tns << "\n";
            }

            //	<-----------------------	Terminating condition		----------------------->	//
            // if (epoch_counter > max_iter || f_value < ao_tol)
        ...
        Status operator()(Options const &options)
        {
          ...
          // c := percentage (%) of dataset to be sampled.
          // blocksize(mode) := c * ( nnz /  tns_dimensions(mode) )
          for (std::size_t i = 0; i < TnsSize; i++)
          {
            mv.blocksize[i] = int(mv.c_stochastic_perc * options.nnz / mv.tnsDims[i]) + 1;
          }
                          ...
                          Options      const &options)
        {
          ...
          for (std::size_t i = 0; i < TnsSize; i++)
          {
            mv.blocksize[i] = int(mv.c_stochastic_perc * options.nonZeros / mv.tnsDims[i]) + 1;
          }
...
#include "GtcStochasticOpenMP.hpp"
#endif /* USE_OPENMP */

namespace partensor
{
  /**
   * Interface of Stochastic General Tensor Completion(gtc_stochastic), with the use of an 
  ...
  gtc_stochastic( ExecutionPolicy                                                                                     &&, 
       SparseOptions<_TnsSize,execution::execution_policy_t<ExecutionPolicy>,SparseDefaultValues>    const &options  )
  {
    ...
    {
      return internal::GTC_STOCHASTIC<_TnsSize>()(options);
    }
    else if constexpr (std::is_same_v<ExPolicy,execution::openmpi_policy>)
    {
      return internal::GTC_STOCHASTIC<_TnsSize,execution::openmpi_policy>()(options);
    }
    else if constexpr (std::is_same_v<ExPolicy,execution::openmp_policy>)
    {
      return internal::GTC_STOCHASTIC<_TnsSize,execution::openmp_policy>()(options);
    }
    else
      return internal::GTC_STOCHASTIC<_TnsSize>()(options);
  }
  ...
  /*
   * Interface of Stochastic General Tensor Completion(gtc_stochastic). Sequential Policy.
   * 
   * @tparam Tensor_      Type(data type and order) of input Tensor.
   *                      @c Tensor_ must be @c partensor::Tensor<order>, where 
   *                      @c order must be in range of @c [3-8].
  ...
  auto gtc_stochastic(SparseOptions<_TnsSize> const &options )
  {
    return internal::GTC_STOCHASTIC<_TnsSize,execution::sequenced_policy>()(options);
  }

  /**
   * Interface of General Tensor Completion(gtc_stochastic), with the use of an 
   * @c Execution @c Policy, which can be either @c sequential, @c parallel 
   * with the use of @c MPI, or @c parallel with the use of @c OpenMP. 
   * In order to choose a policy, type @c execution::seq, @c execution::mpi or 
   * @c execution::omp. Default value is @c sequential, in case no @c ExecutionPolicy
       ...
       SparseOptions<_TnsSize,execution::execution_policy_t<ExecutionPolicy>,SparseDefaultValues>    const &options  )
  {
    ...
    if constexpr (std::is_same_v<ExPolicy,execution::sequenced_policy>)
    {
      return internal::GTC_STOCHASTIC<_TnsSize>()(Ratings_Base_T,options);
    }
    else if constexpr (std::is_same_v<ExPolicy,execution::openmpi_policy>)
    {
      return internal::GTC_STOCHASTIC<_TnsSize,execution::openmpi_policy>()(Ratings_Base_T,options);
    }
    else if constexpr (std::is_same_v<ExPolicy,execution::openmp_policy>)
    {
      return internal::GTC_STOCHASTIC<_TnsSize,execution::openmp_policy>()(Ratings_Base_T,options);
    }
    else
      return internal::GTC_STOCHASTIC<_TnsSize>()(Ratings_Base_T,options);
  }
   ...
   */
  template<std::size_t _TnsSize>
  auto gtc_stochastic(Matrix                              const &Ratings_Base_T, 
                      SparseOptions<_TnsSize>             const &options )
  {
    return internal::GTC_STOCHASTIC<_TnsSize,execution::sequenced_policy>()(Ratings_Base_T,options);
  }

}

#endif // PARTENSOR_GTC_STOCHASTIC_HPPP