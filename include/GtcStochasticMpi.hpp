...
#error "GTC_STOCHASTIC_MPI can only included inside GTC"
...
namespace partensor
{
  ...
  inline namespace v1 {
    ...
    namespace internal {
      /**
       * Includes the implementation of GTCSTOCHASTICMPI factorization. Based on the given
      ...
      struct GTC_STOCHASTIC<TnsSize_,execution::openmpi_policy> : public GTC_STOCHASTIC_Base<TnsSize_>
      {
        ...
        void NesterovMNLS_stochastic( Member_Variables                   &mv, 
                                      Status                      const  &st,
                                      int    const                        idx)
        {
          int iter = 0;
          ...
          int rows_layer = mv.subTnsDims[idx][mv.fiber_rank[idx]];

			  	Matrix inv_L2(rows_layer, 1); // rows_layer
          
          Matrix grad_Y_T(mv.rank, rows_layer);
          Matrix grad_Y_local_T(mv.rank, mv.rows_for_update[idx]);

          Matrix Y_T(mv.rank, rows_layer);
          ...
          Matrix A(mv.rank, mv.rows_for_update[idx]);
				 ...
				 const Matrix zero_mat = Matrix::Zero(mv.rank, mv.rank);
          ...
          while (1)
          {
            ...
            for (long int i = 0; i < mv.subTns[idx].outerSize(); ++i)
            {
              ...
              SparseMatrix::InnerIterator it(mv.subTns[idx], i);

              // Get the number of nnz per row of matricization.
              long int nnzs_per_col = mv.subTns[idx].innerVector(i).nonZeros();

						  long int var_blocksize_i = static_cast<long int>(mv.c_stochastic_perc * nnzs_per_col);

              if (var_blocksize_i > 0)
              {
                // Choose a pivot from [0, nnzs_per_col - blocksize].
                long int pivot = (var_blocksize_i > nnzs_per_col) ? 0 : (std::rand() % (nnzs_per_col - var_blocksize_i + 1));

                SparseMatrix::InnerIterator it(mv.subTns[idx], i);

                // Iterate over [pivot, pivot + blocksize] nnz elements per row.
                // it += pivot;
                ...
                for (long int sample = 0; sample < var_blocksize_i; sample++, ++it)
                {
                  ...
                  temp_col += ((temp_R_1.transpose() * Y_T.col(i))(0)  - it.value()) * temp_R_1;
            ...
            // Add proximal term.
            grad_Y_local_T += lambda * (Y_local_T + A);
                           ...
                           Member_Variables       &mv  )
        {
          switch ( st.options.constraints[idx] ) 
          {
            ...
            {
              NesterovMNLS_stochastic(mv, st, idx);
              break;
            }
         ...
         */
        void aogtc_stochastic(CartCommunicator const &grid_comm,
                   Member_Variables       &mv,
                   Status                 &status)
        {
        ...
        Status operator()(Options const &options)
        {
         ...
         // Communicator with cartesian topology
          ...
          aogtc_stochastic(grid_comm, mv, status);
