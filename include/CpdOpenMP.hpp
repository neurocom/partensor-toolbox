...
* @file      CpdOpenMP.hpp
* @details
* Implements the Canonical Polyadic Decomposition(cpd) using Shared
* memory and @c OpenMP.
...
#error "CPD_OPENMP can only included inside CPD"
...
namespace partensor {
  ...
  inline namespace v1 {
    ...
    namespace internal {
        /**
         * Includes the implementation of CPDOPENMP factorization. Based on the 
        ...
        struct CPD<Tensor_,execution::openmp_policy> : public CPD_Base<Tensor_>
        {
            ...
            using Options = partensor::Options<Tensor_,execution::openmp_policy,DefaultValues>;
            using Status  = partensor::Status<Tensor_,execution::openmp_policy,DefaultValues>;
            ...
            */
                     ...
                     Status                 &status)
            {
                ...
                while (1)
                {
                    ...
                    for (std::size_t i=0; i<TnsSize; i++)
                    {
                        ...
                        mttkrp(mv.tnsDims, status.factors,  mv.tns_mat[i], i, get_num_threads(), mv.mttkrp[i]);
            ...
            Status operator()(Tensor_     const &tnsX, 
                              std::size_t const  R)
            {
              Options          options = MakeOptions<Tensor_,execution::openmp_policy,DefaultValues>();
