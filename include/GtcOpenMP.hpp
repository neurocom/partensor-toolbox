...
#error "GTC_OMP can only included inside GTC"
...
namespace partensor
{
  inline namespace v1
  {
    namespace internal
    {
      ...
      struct GTC<TnsSize_, execution::openmp_policy> : public GTC_Base<TnsSize_>
      {
                           ...
                           Status                 &st   )
        {
          ...
          switch ( st.options.constraints[idx] ) 
          {
            ...
            {
              int last_mode = (idx == static_cast<int>(TnsSize) - 1) ? static_cast<int>(TnsSize) - 2 : static_cast<int>(TnsSize) - 1;
              SparseMTTKRP_omp(mv.tnsDims, mv.tnsX[idx], mv.factors_T, mv.rank, mv.offsets[idx], last_mode, idx, mv.mttkrp_T[idx]);
              #pragma omp barrier

              // NesterovMNLS(mv.cwise_factor_product, mv.factors_T, mv.tnsDims, mv.tnsX[idx], mv.offsets[idx], mv.Y[idx], 
              //              st.options.max_nesterov_iter, st.options.lambdas[idx], idx, st.options.constraints[idx], mv.mttkrp_T[idx]);

              local_L::NesterovMNLS(mv.invL[idx], mv.factors_T, mv.tnsDims, mv.tnsX[idx], mv.offsets[idx], mv.Y[idx], 
                           st.options.max_nesterov_iter, st.options.lambdas[idx], idx, mv.mttkrp_T[idx]);
          ...
          #pragma omp master
          {
            ...
            mv.factor_T_factor[idx].noalias() = mv.factors_T[idx] * mv.factors_T[idx].transpose();
          }
        }
                   ...
                   Status                 &status)
        {
          ...
          for (std::size_t i=0; i<TnsSize; i++)
          {
            ...
            mv.invL[i]         = Matrix::Zero(mv.tnsDims[i], 1);
          }
                  ...
                  shared(status, mv)
          {
            ...
            while(1)
            {
                ...
                #pragma omp barrier
                // Avg Factors if symmetric tensor
                if (status.options.averaging && status.ao_iter >= 1)
                {
                  #pragma omp master
                  {
                    for (int i = 1; i < static_cast<int>(TnsSize); i++)
                    {
                      mv.factors_T[0].noalias() += mv.factors_T[i];
                    }

                    mv.factors_T[0].noalias() = mv.factors_T[0] / TnsSize;

                    for (int i = 0; i < static_cast<int>(TnsSize); i++)
                    {
                      mv.factors_T[i] = mv.factors_T[0];

                      status.factors[i] = mv.factors_T[i].transpose();
                ...
                #pragma omp barrier // DON'T REMOVE!
        ...
        Status operator()(Options const &options)
        {
          ...
          // GTC_Base<TnsSize>::sort_ratings_base(mv.Ratings_Base_T, options.nonZeros);
                          ...
                          Options      const &options)
        {
          ...
          std::cout << "GtcOpenMP took " << end_gtc_time_omp << " sec." << std::endl;
          return status;
        }
      };
    }  // namespace internal
